...
function createActionGroup(config2) {
  const {
    source,
    events
  } = config2;
...
function provideState(featureNameOrSlice, reducers, config2 = {}) {
  return makeEnvironmentProviders([..._provideState(featureNameOrSlice, reducers, config2), ENVIRONMENT_STATE_PROVIDER]);
}
...
function createEffect(source, config2 = {}) {
  const effect2 = config2.functional ? source : source();
...
var _StoreDevtools = class _StoreDevtools {
  constructor(dispatcher, actions$, reducers$, extension, scannedActions, errorHandler, initialState2, config2) {
    const liftedInitialState = liftInitialState(initialState2, config2.monitor);
    const liftReducer = liftReducerWith(initialState2, liftedInitialState, errorHandler, config2.monitor, config2);
...
var BoardComponent_Defer_3_DepsFn = () => [NgClass, DefaultValueAccessor, NumberValueAccessor, NgControlStatus, FormControlName, import("./chunk-MELX6S77.mjs").then((m) => m.InputValidatorDirective)];
...
@angular/forms/fesm2022/forms.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-HXOX3MGB.mjs.map
